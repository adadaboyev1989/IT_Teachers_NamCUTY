...
import { Plus, Trash2, Edit3, Upload, Download, FileSpreadsheet, X, Loader2, Search, Award, Clock, Calendar } from 'lucide-react'
...
export function AdminPedagog() {
  ...
  const handleDownloadTemplate = () => {
    const template = [
      {
        'FIO': 'Ali Valiyev',
        'Maktabi': 'Namangan 1-maktab',
        'PINFL': '12345678901234',
        'Tug\'ilgan sanasi': '1985-03-15',
        'Toifasi': 'Oliy',
        'Dars soati': 18,
        'Sertifikat nomi': 'Microsoft Certified Educator',
        'Sertifikat olingan sana': '2023-06-01',
        'Sertifikat tugash sanasi': '2026-06-01',
      },
      {
        'FIO': 'Vali Aliyev',
        'Maktabi': 'Namangan 2-maktab',
        'PINFL': '98765432109876',
        'Tug\'ilgan sanasi': '1990-07-20',
        'Toifasi': 'Birinchi',
        'Dars soati': 12,
        'Sertifikat nomi': '',
        'Sertifikat olingan sana': '',
        'Sertifikat tugash sanasi': '',
      },
    ]
  ...
  const handleExportExcel = () => {
    ...
    const exportData = data.map((d) => ({
      ...
      'Sertifikat olingan sana': d.certificate_issue_date || '',
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const pedagogRows = rows.map((row) => {
        ...
        const rawPinfl = getVal(['PINFL', 'Pinfl'])
        ...
        return {
          ...
          pinfl: validPinfl,
          birth_date: getVal(['Tug\'ilgan sanasi', 'Tugilgan sanasi', 'Birth date', 'Birth']) || null,
          category: validCategory,
          lesson_hours: parseInt(getVal(['Dars soati', 'Dars soat', 'Lesson hours', 'Hours'])) || 0,
          certificate_name: getVal(['Sertifikat nomi', 'Certificate name']) || null,
          certificate_issue_date: getVal(['Sertifikat olingan sana', 'Certificate issue date']) || null,
      ...
      // Same PINFL can appear more than once (a corrected re-upload of the
      // same file, overlapping exports, etc). pedagog_data.pinfl is unique in
      // the database, so upsert-by-pinfl updates the existing record instead
      // of failing the whole batch with a constraint violation; rows without
      // a PINFL always insert as new (NULLs never conflict).
  ...
  const filtered = data.filter((d) => {
    ...
    return (
      ...
      (d.pinfl || '').toLowerCase().includes(q)
    )
  })
  ...
  return (
    <div>
      <SectionHeader
        title="Pedagog ma'lumotlari"
        subtitle="O'qituvchilarning pedagogik ma'lumotlarini boshqarish"
        onAdd={() => { setEditing(null); setShowForm(true) }}
        addLabel="Pedagog qo'shish"
      />
      ...
      <div className="mb-5 flex flex-wrap gap-2">
        ...
        <button onClick={handleExportExcel} disabled={data.length === 0} className="btn-ghost disabled:opacity-50">
      ...
      {importMsg && (
        <div className={`mb-5 rounded-lg px-4 py-3 text-sm ${
          importMsg.includes('xatosi') || importMsg.includes('topilmadi') || importMsg.includes('o\'qib')
            ? 'bg-red-50 text-red-600'
            : 'bg-emerald-50 text-emerald-600'
        }`}>
          {importMsg}
          <button onClick={() => setImportMsg(null)} className="ml-3 text-current opacity-60 hover:opacity-100">
            <X className="inline h-3.5 w-3.5" />
          </button>
        </div>
      )}

      <SearchBar value={search} onChange={setSearch} placeholder="FIO, maktab, toifa yoki PINFL bo'yicha qidirish..." />
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-neutral-200 bg-neutral-50">
                <tr>
                  ...
                  <th className="hidden px-4 py-3 font-semibold text-neutral-700 lg:table-cell">PINFL</th>
                  ...
                  <tr key={p.id} className="transition-colors hover:bg-neutral-50">
                    <td className="px-4 py-3">
                      ...
                      <div className="mt-1 sm:hidden">
                        <span className={`badge ${categoryColors[p.category] || 'bg-neutral-100 text-neutral-600'}`}>
                          {p.category}
                        </span>
                      </div>
                    </td>
                      ...
                      {p.certificate_name ? (
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="inline-flex items-center gap-1 text-xs text-neutral-400">
                            ...
                            {formatDate(p.certificate_issue_date)} — {formatDate(p.certificate_expiry_date)}
                          </span>
                        </div>
                      ) : (
    ...
    if (upsertError) {
      setError(
        upsertError.code === '23505'
          ? 'Bu PINFL raqami bilan boshqa pedagog allaqachon mavjud.'
    ...
    <Modal title={pedagog ? 'Pedagog ma\'lumotini tahrirlash' : 'Pedagog qo\'shish'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <FormField label="PINFL raqami">
            <input type="text" value={pinfl} onChange={(e) => setPinfl(e.target.value)} className="input-field" placeholder="12345678901234" maxLength={14} />
          </FormField>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <FormField label="Dars soati">
            <input type="number" value={lessonHours} onChange={(e) => setLessonHours(e.target.value)} className="input-field" placeholder="0" min={0} />
          </FormField>
        </div>

        <div className="border-t border-neutral-100 pt-4">
          <p className="mb-3 text-sm font-semibold text-neutral-700">Sertifikat ma'lumotlari (majburiy emas)</p>
          <FormField label="Sertifikat nomi">
            <input type="text" value={certName} onChange={(e) => setCertName(e.target.value)} className="input-field" placeholder="Microsoft Certified Educator" />
          </FormField>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <FormField label="Sertifikat olingan sana">
              <input type="date" value={certIssue} onChange={(e) => setCertIssue(e.target.value)} className="input-field" />
            </FormField>
            <FormField label="Sertifikat tugash sanasi">
              <input type="date" value={certExpiry} onChange={(e) => setCertExpiry(e.target.value)} className="input-field" />
            </FormField>
          </div>
        </div>
